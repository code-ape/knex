...
class Client_SQLite3 extends Client {
  ...
  processResponse(obj, runner) {
    ...
    switch (obj.method) {
      ...
      case 'insert': {
        if (returning) {
          // If Sqlite supports returning, as supported for version 3.35.0 and up.
          if (isSqlite3DriverOfVersionOrHigher([3, 35, 0], this.driver)) {
            ...
            this.logger.warn(
              'sqlite3 must be version 3.35.0 or higher to support RETURNING clause'
            );
          }
        }
        return [ctx.lastID];
