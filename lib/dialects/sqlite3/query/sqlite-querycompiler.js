...
const { isSqlite3DriverOfVersionOrHigher } = require('./utils')
...
class QueryCompiler_SQLite3 extends QueryCompiler {
  ...
  insert() {
    ...
    // Proactively handle all 'empty insert' cases.
    ...
    // If raw insert then return as is
    ...
    // Proactively handle when insert data has no columns
    ...
    // Proactively handle if there's a single insert value 
    ...
    // If need to construct block based UNION insert
    const blocks = [];
    let i = -1;
